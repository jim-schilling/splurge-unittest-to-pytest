...
class BuildFixtureSpecsTask(Task):
    ...
    def execute(self, context: Mapping[str, Any], resources: Any) -> TaskResult:  # type: ignore[override]
        ...
        # Precompute bundling (full) so we can skip per-attr fixtures that are bundled
        maybe = safe_bundle_named_locals(out.classes, set(used_local_names), full=True)
        if len(maybe) == 3:
        ...
        # Second pass for bundling to produce wrapper nodes using default (non-full)
        prepend_nodes2, bundler_typing2, bundled_attr_map2 = safe_bundle_named_locals(
            out.classes, set(module_level_names)
        )
        ...
        return TaskResult(
            delta=ContextDelta(
                values={
                    ...
                    "gen_fixture_nodes": fixture_nodes,
                    "gen_specs": specs,
