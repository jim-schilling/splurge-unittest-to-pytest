...
class DetectNeedsCstTask(Task):
    ...
    def execute(self, context: Mapping[str, Any], resources: Any) -> TaskResult:  # type: ignore[override]
        ...
        # Define Steps inline to preserve locality and avoid public API churn
        @dataclass
        class _DetectNeedsStep:
    ...
    def execute(self, context: Mapping[str, Any], resources: Any) -> TaskResult:  # type: ignore[override]
        ...
        class _InsertImportsStep:
            ...
            def execute(self, ctx: Mapping[str, Any], resources: Any) -> StepResult:
                ...
                re_import_node = cst.SimpleStatementLine(
                    body=[cst.Import(names=[cst.ImportAlias(name=cst.Name("re"))])]
                )
                sys_import_node = cst.SimpleStatementLine(
                    body=[cst.Import(names=[cst.ImportAlias(name=cst.Name("sys"))])]
                )
                os_import_node = cst.SimpleStatementLine(
                ...
                if typing_needed:
                    ...
                    if "Path" in missing:
                        ...
                        pathlib_import_node = cst.SimpleStatementLine(
                            body=[
                                cst.ImportFrom(
                                    module=cst.Name("pathlib"), names=[cst.ImportAlias(name=cst.Name("Path"))]
                                )
                            ]
                        )
                        have_pathlib = False
                        for stmt in mod.body:
                            if isinstance(stmt, cst.SimpleStatementLine) and stmt.body:
                                ...
                                if (
                                    ...
                                    and getattr(first.module, "value", None) == "pathlib"
                                ):
                                    have_pathlib = True
                                    break
                        if not have_pathlib:
