            ...
            def _body_calls_main(stmt_block: cst.BaseSuite) -> bool:
                for s in getattr(stmt_block, 'body', []):
                    ...
                    if isinstance(s, cst.SimpleStatementLine) and s.body:
                        ...
                        if isinstance(first_small, cst.Expr) and isinstance(getattr(first_small, 'value', None), cst.Call):
                            # mypy thinks first_small.value is BaseExpression; cast after isinstance
                            call_node = cast(cst.Call, first_small.value)
                        elif isinstance(first_small, cst.Assign) and isinstance(getattr(first_small, 'value', None), cst.Call):
                            call_node = cast(cst.Call, first_small.value)
                    ...
                    if call_node is not None:
                        ...
                        if isinstance(func, cst.Name) and func.value == 'main':
                            return True
                        if isinstance(func, cst.Attribute) and isinstance(getattr(func, 'attr', None), cst.Name) and func.attr.value == 'main':
                            return True
                        ...
                        for a in getattr(call_node, 'args', []) or []:
                            ...
                            if isinstance(aval, cst.Call):
                                ...
                                if isinstance(afunc, cst.Name) and afunc.value == 'main':
                                    return True
                                if isinstance(afunc, cst.Attribute) and getattr(afunc, 'attr', None) and isinstance(afunc.attr, cst.Name) and afunc.attr.value == 'main':
                                    return True
