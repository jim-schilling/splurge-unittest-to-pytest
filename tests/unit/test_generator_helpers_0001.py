...
def test_cleanup_needs_shutil_true_for_shutil_call():
    stmt = cst.SimpleStatementLine(
        body=[cst.Expr(cst.Call(func=cst.Attribute(value=cst.Name("shutil"), attr=cst.Name("rmtree")), args=[]))]
    )
...
def test_cleanup_needs_shutil_false_for_unrelated():
    stmt = cst.SimpleStatementLine(
        body=[cst.Expr(cst.Call(func=cst.Name("print"), args=[cst.Arg(cst.SimpleString('"x"'))]))]
    )
