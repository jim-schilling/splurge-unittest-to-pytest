...
def test_fixture_injector_inserts_nodes_and_autouse_when_unittest_used() -> None:
    ...
    fn = cst.FunctionDef(name=cst.Name("x"), params=cst.Parameters(), body=cst.IndentedBlock(body=[cst.SimpleStatementLine(body=[cst.Return(cst.Integer("42"))])]), decorators=[cst.Decorator(decorator=cst.Attribute(value=cst.Name("pytest"), attr=cst.Name("fixture")))])
