...
def test_fixture_injector_inserts_fixture_after_pytest_import_and_adds_autouse_when_unittest():
    ...
    fn = cst.FunctionDef(name=cst.Name('a'), params=cst.Parameters(), body=cst.IndentedBlock(body=[cst.Pass()]), decorators=[cst.Decorator(decorator=cst.Attribute(value=cst.Name('pytest'), attr=cst.Name('fixture')))])
    # CollectorOutput with has_unittest_usage True to force autouse attach
    collector = CollectorOutput(module=module, module_docstring_index=None, imports=[], classes={}, has_unittest_usage=True)
    res = fixture_injector.fixture_injector_stage({'module': module, 'fixture_nodes': [fn], 'collector_output': collector})
    new_mod = res['module']
    code = new_mod.code
