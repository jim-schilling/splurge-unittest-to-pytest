...
def test_build_fixture_body_with_simple_literal_preserves_cleanup():
    ...
    cleanup = [
        cst.SimpleStatementLine(body=[cst.Expr(value=cst.Call(func=cst.Name("cleanup_fn"), args=[]))])
    ]

    body = build_fixture_body("attr", value_expr, cleanup)
    assert isinstance(body, cst.IndentedBlock)
    stmts = body.body
    # first statement should be a yield of the literal
...
def test_build_fixture_body_with_complex_value_assigns_and_rewrites_cleanup():
    ...
    # cleanup statements reference both self.myattr and bare myattr (name)
    cleanup = [
    ...
    # first statement is assignment to local name
    assert isinstance(stmts[0], cst.SimpleStatementLine)
    ...
    # second statement is yield of that name
    ...
    # cleanup statements should have had attribute/name replaced with the local name
    # collect Name occurrences
    ...
    # cleanup statements start at index 2
    for s in stmts[2:]:
        s.visit(collector)

    # all referenced names in cleanup should be replaced to expected_name
