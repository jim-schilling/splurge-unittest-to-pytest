...
def make_collector_out(setup_assignments, local_assignments=None, teardown_statements=None):
    ...
    class_node = cst.ClassDef(name=cst.Name("MyTest"), body=cst.IndentedBlock(body=[cst.SimpleStatementLine(body=[cst.Pass()])]))
...
def test_infers_filename_from_positional_literal():
    ...
    setup = {"sql_file": cst.Call(func=cst.Name("str"), args=[cst.Arg(value=cst.Name("sql_file"))]), "sql_content": cst.SimpleString('"create"')}
...
def test_infers_filename_from_keyword_arg():
    helper_call = cst.Call(func=cst.Name("some_helper"), args=[cst.Arg(keyword=cst.Name("filename"), value=cst.SimpleString('"data.json"'))])
    local_map = {"data_file": (helper_call, 0)}
    setup = {"data_file": cst.Call(func=cst.Name("str"), args=[cst.Arg(value=cst.Name("data_file"))]), "data_content": cst.SimpleString('"x"')}
    co = make_collector_out(setup, local_map)
    context = {"collector_output": co, "module": cst.Module(body=[]), "autocreate": True}
    code = render_fixture_nodes_from_stage(context)
...
def test_infers_filename_from_path_constructor():
    path_call = cst.Call(func=cst.Attribute(value=cst.Name("pathlib"), attr=cst.Name("Path")), args=[cst.Arg(value=cst.SimpleString('"file.sql"'))])
    local_map = {"file_file": (path_call, 0)}
    setup = {"file_file": cst.Call(func=cst.Name("str"), args=[cst.Arg(value=cst.Name("file_file"))]), "file_content": cst.SimpleString('"ok"')}
    co = make_collector_out(setup, local_map)
    context = {"collector_output": co, "module": cst.Module(body=[]), "autocreate": True}
    code = render_fixture_nodes_from_stage(context)
