...
def test_directory_without_recursive_warns_and_no_files(tmp_path, monkeypatch):
    # create a directory and run without --recursive; expect warning and exit 0 message "No unittest files found"
    d = tmp_path / "dir"
    d.mkdir()
    runner = CliRunner()

    # monkeypatch Path.is_file/is_dir to behave normally; run CLI
...
def test_dry_run_calls_convert_string_and_reports(monkeypatch, tmp_path):
    ...
    # conversion result has_changes False and not verbose: CLI prints only the summary
    assert result.exit_code == 0
...
def test_convert_file_error_is_reported(monkeypatch, tmp_path):
    ...
    # convert_file returned errors so CLI should exit with status 1 and report errors
    assert result.exit_code == 1
