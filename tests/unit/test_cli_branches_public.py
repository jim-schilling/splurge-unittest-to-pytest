...
def test_dry_run_verbose_shows_diff_and_summary(monkeypatch, tmp_path):
    ...
    # make convert_string return has_changes=True with added pytest import
    ...
    # cli.main imports convert_string from splurge_unittest_to_pytest.main inside the function,
    # so patch the symbol on that module so the local import resolves to our fake.
...
def test_backup_creation_and_warning_on_failure(monkeypatch, tmp_path):
    ...
    # fake convert_file to return has_changes True
    ...
    # Ensure shutil.copy2 will raise to hit the warning branch
    ...
    runner = CliRunner()
    # non-dry-run so backup is attempted
