...
def test_extract_relevant_cleanup_assign_and_call():
    ...
    assign = cst.SimpleStatementLine(
        body=[cst.Assign(targets=[cst.AssignTarget(target=cst.Name("a"))], value=cst.Name("foo"))]
    )
    # self.x.cleanup()
    call = cst.SimpleStatementLine(
        body=[
            cst.Expr(
                value=cst.Call(
                    func=cst.Attribute(
                        value=cst.Attribute(value=cst.Name("self"), attr=cst.Name("x")), attr=cst.Name("cleanup")
                    ),
                    args=[],
                )
            )
...
def test_references_attribute_in_call_args_and_subscript():
    ...
    inner_call = cst.Call(
        func=cst.Name("bar"), args=[cst.Arg(value=cst.Attribute(value=cst.Name("self"), attr=cst.Name("x")))]
    )
    call_stmt = cst.SimpleStatementLine(body=[cst.Expr(value=inner_call)])

    sub = cst.Subscript(
        value=cst.Name("arr"),
        slice=[cst.SubscriptElement(slice=cst.Index(value=cst.Attribute(value=cst.Name("self"), attr=cst.Name("x"))))],
    )
...
def test_extract_relevant_cleanup_if_and_indentedblock():
    ...
    if_stmt = cst.If(
        test=cst.Attribute(value=cst.Name("self"), attr=cst.Name("x")),
        body=cst.IndentedBlock(
            body=[cst.SimpleStatementLine(body=[cst.Expr(value=cst.Call(func=cst.Name("inner_call"), args=[]))])]
        ),
    )

    # orelse as IndentedBlock
    if_with_orelse = cst.If(
        test=cst.Name("flag"),
        body=cst.IndentedBlock(
            body=[
                cst.SimpleStatementLine(
                    body=[cst.Assign(targets=[cst.AssignTarget(target=cst.Name("a"))], value=cst.Name("b"))]
                )
            ]
        ),
        orelse=cst.Else(
                ...
                body=[
                    cst.SimpleStatementLine(
                        body=[
                            cst.Expr(
                                value=cst.Call(
                                    func=cst.Attribute(value=cst.Name("self"), attr=cst.Name("cleanup")), args=[]
                                )
                            )
                        ]
                    )
