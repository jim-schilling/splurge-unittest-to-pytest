...
def test_create_simple_fixture_structure():
    ...
    # Body may be a single direct Return or an Assign followed by Return
    assert isinstance(f.body, cst.IndentedBlock)
    assert len(f.body.body) >= 1
    # if two or more statements, expect first to be Assign and a Return present; otherwise allow direct Return
    if len(f.body.body) >= 2:
        ...
        # single statement: must be a Return
        assert isinstance(f.body.body[0].body[0], cst.Return)
...
def test_create_fixture_with_cleanup_yield_and_replacement():
    ...
    assert any(
        isinstance(s.body[0], cst.Expr) and isinstance(s.body[0].value, cst.Yield)
        for s in stmts
