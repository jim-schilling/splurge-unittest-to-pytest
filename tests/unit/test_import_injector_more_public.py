...
def test_explicit_flags_but_not_needed_returns_module():
    # Caller set flags but module text does not require pytest or re -> return unchanged
    src = "x = 1\n"
    m = make_module(src)
    out = import_injector.import_injector_stage({"module": m, "needs_pytest_import": False, "needs_re_import": False})
    # Should return the original module (no 'module' key since it returns early)
...
def test_unittest_already_imported_prevents_unittest_insertion():
    ...
    # ensure we did not add another unittest import (no duplicate)
    imports = [s for s in new_mod.body if isinstance(s, cst.SimpleStatementLine) and isinstance(s.body[0], (cst.Import, cst.ImportFrom))]
    unittest_count = 0
    for s in imports:
        first = s.body[0]
...
def test_preferred_order_inserts_shutil_and_json_after_pytest():
    src = '"""docstring"""\n\n# placeholder\n'
    m = make_module(src)
    out = import_injector.import_injector_stage({"module": m, "needs_shutil_import": True, "needs_re_import": True, "needs_pytest_import": True,})
    new_mod = out["module"]
    # Collect the sequence of top-level import module names (dots for ImportFrom)
    names = []
    ...
    # Ensure pytest appears before shutil and json (preferred ordering)
...
def test_pathlib_deduplication_when_existing():
    # If pathlib import already exists, requesting Path should not add duplicate
    src = 'from pathlib import Path\n\nx=1\n'
    m = make_module(src)
    out = import_injector.import_injector_stage({"module": m, "needs_typing_names": ["Path"]})
    new_mod = out.get("module", m)
    # count pathlib ImportFrom occurrences
    pathlib_count = 0
    for s in new_mod.body:
        if isinstance(s, cst.SimpleStatementLine) and s.body and isinstance(s.body[0], cst.ImportFrom) and getattr(s.body[0].module, "value", None) == "pathlib":
            pathlib_count += 1
