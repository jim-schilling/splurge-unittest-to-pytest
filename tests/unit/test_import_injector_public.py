...
def test_default_inserts_pytest_when_no_flags():
    ...
    new_mod = out["module"]
    # first statement should be import pytest because no imports/docstring present
...
def test_inserts_after_docstring():
    ...
    new_mod = out["module"]
    # import pytest should appear after docstring and existing import
...
def test_does_not_duplicate_existing_imports():
    ...
    out = import_injector.import_injector_stage({"module": m, "needs_pytest_import": True})
    new_mod = out["module"]
    # ensure only single pytest import remains
    pytest_imports = [s for s in new_mod.body if isinstance(s, cst.SimpleStatementLine) and isinstance(s.body[0], cst.Import) and s.body[0].names[0].name.value == "pytest"]
...
def test_typing_and_pathlib_insertion_and_merge():
    ...
    new_mod = out["module"]
    # should insert pathlib.Path and from typing import Any (Path handled separately)
    found_pathlib = any(isinstance(s.body[0], cst.ImportFrom) and getattr(s.body[0].module, "value", None) == "pathlib" for s in new_mod.body if isinstance(s, cst.SimpleStatementLine))
    assert found_pathlib
    # typing import should include Any
    typing_stmt = None
    for s in new_mod.body:
        if isinstance(s, cst.SimpleStatementLine) and isinstance(s.body[0], cst.ImportFrom) and getattr(s.body[0].module, "value", None) == "typing":
            typing_stmt = s
            break
...
def test_detects_pytest_usage_in_module_text():
    ...
    # note: caller doesn't set needs_pytest_import -> default True will already insert
    m = make_module(src)
    out = import_injector.import_injector_stage({"module": m, "needs_pytest_import": False})
    # should insert pytest because module text contains pytest.
    new_mod = out["module"]
    assert any(isinstance(s.body[0], cst.Import) and s.body[0].names[0].name.value == "pytest" for s in new_mod.body if isinstance(s, cst.SimpleStatementLine))
